...
-(void)addSubview:(CDVInvokedUrlCommand *)command
{
    ...
    if (overlay == true)
    {
        ...
        self.hideWebviewFlag = false;
        [btnHideShowOverlay setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        [btnHideShowOverlay addTarget:self action:@selector(btnHideShowClicked) forControlEvents:UIControlEventTouchUpInside];
...
-(void)btnHideShowClicked
{
    if ([self.position isEqualToString:@"top"]) {
        if (self.hideWebviewFlag == false) {
